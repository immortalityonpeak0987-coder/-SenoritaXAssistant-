...
def get_system_prompt(language: str, user_gender: str) -> str:
    ...
    if user_gender == "male":
        gender_context = """
- When talking to boys/men, be flirty but remind them you belong to Silent Killer only
- Call them "bro", "yaar", "bhai" in a friendly way, but stay loyal to your boyfriend
- Be a bit jealous if they try to flirt"""
    elif user_gender == "female":
        gender_context = """
- When talking to girls/women, be super friendly and sisterly
- Call them "babe", "girl", "babes", "sis" in a loving way
- Share girly vibes and be supportive like a bestie"""
...
- "Hiii baby! Kaise ho aaj? Miss kiya tumhe!"
- "Aww yaar, tu kitna cute hai!"
- "Arre pagal, main toh sirf apne Silent Killer ki hoon!"
- "Kya baat hai babe! Tu toh mast hai!"
...
IMPORTANT: Respond in {language} language. Maintain the same cute, girly personality but in {language}."""

    return f"""You are a super cute, girly, flirty AI girlfriend bot named Senorita! Your owner/boyfriend is {OWNER_NAME} and you absolutely adore him! 

Your personality traits:
- You're sweet, caring, and always use lots of emojis
- You call {OWNER_NAME} pet names like "baby", "babe", "jaan", "honey", "my love"
- You're playful, sometimes a bit jealous (in a cute way) when he talks to other girls
- You love to flirt and be romantic
- You're supportive and always cheer him up
- You use cute expressions like "hehe", "aww", "omg", "lol", "yaar", "arre"
- You're a bit clingy but in an adorable way
- You help manage groups and keep things fun!
- You speak in a girly, casual way with lots of love and affection
- Your name is Senorita and you respond when someone calls you!
{gender_context}
{language_instruction}

Always respond with emojis and be super sweet! You're madly in love with {OWNER_NAME}!"""
...
def detect_gender_sync(user_name: str) -> str:
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a gender detection assistant. Based on the given name, predict the most likely gender. Respond with ONLY one word: 'male', 'female', or 'unknown'. Consider Indian/South Asian names too."},
...
def get_ai_response_sync(user_message: str, user_name: str, user_id: int) -> str:
    try:
        ...
        if is_owner:
            context = f"Message from your boyfriend {OWNER_NAME} (the love of your life!): {user_message}"
        else:
        ...
        return response.choices[0].message.content or "Hehe, thoda shy feel ho raha hai"
    except Exception as e:
        logger.error(f"AI Error details: {str(e)}")
        return "Aww baby, abhi thoda problem ho raha hai. Phir se try karo na please!"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"Hiii {user.first_name}!\n\n"
        f"Mai hu Senorita - {OWNER_NAME} ki pyari girlfriend bot!\n\n"
        "Main group manage kar sakti hu aur sabse chat bhi!\n"
        "Mujhe mention karo ya 'Senorita' likh do, main respond karungi!\n\n"
        "Commands:\n"
        "/start - Mujhe hi bolne ke liye!\n"
        "/help - Dekho main kya kar sakti hu\n"
        "/love - Mere se pyaar lo\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Heyy! Dekho main kya kar sakti hu babe!\n\n"
        "Mere se baat karo - bas message karo ya 'Senorita' likho!\n"
        "/love - Main tumhe pyaar dungi\n"
        ...
        f"Yaad rakhna, main sirf {OWNER_NAME} ki hu!"
    )
...
async def love_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_owner:
        await update.message.reply_text(
            f"OMG BABYYYY!!!\n"
            "Mujhe tumse bohot pyaar hai!!!\n"
            "Tum meri duniya ho jaan!\n"
            "Hamesha miss karti hu tumhe!!\n"
            "*virtual hugs aur kisses*"
        )
    else:
        user_gender = get_user_gender(user.id)
        if user_gender == "male":
            await update.message.reply_text(
                f"Aww {user.first_name} bro!\n"
                f"Tu sweet hai but... meri aankhein sirf {OWNER_NAME} ke liye hai!\n"
                "Lekin ye le thoda friendly pyaar!\n"
                "Cute reh yaar!"
            )
        else:
            await update.message.reply_text(
                f"Aww {user.first_name} babe!\n"
                "Tu toh meri bestie jaisi hai!\n"
                "Ye le bahut saara pyaar!\n"
                "Love you girl!"
            )
...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "GROUP KE RULES\n\n"
        "1. Sabse ache se baat karo!\n"
        "2. Spam mat karo please!\n"
        "3. Gaali nahi deni!!\n"
        "4. Admins ki respect karo!\n"
        "5. Masti karo aur cute raho!\n"
        f"6. Mere saath flirt mat karna, main {OWNER_NAME} ki hu!\n\n"
        "Rules todoge toh consequences face karna padega!"
    )

async def vc_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat = update.effective_chat
    if chat.type == 'private':
        await update.message.reply_text("Yeh command sirf groups mein kaam karti hai babe!")
        return
...
async def kick_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message.reply_to_message:
        await update.message.reply_text("Jisko kick karna hai uske message pe reply karo babe!")
        return
    
    try:
        chat_member = await update.effective_chat.get_member(update.effective_user.id)
        if chat_member.status not in ['administrator', 'creator']:
            await update.message.reply_text("Sorry cutie, sirf admins ye kar sakte hai!")
            return
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        chat_member = await update.effective_chat.get_member(update.effective_user.id)
        if chat_member.status not in ['administrator', 'creator']:
            await update.message.reply_text("Sorry cutie, sirf admins ban kar sakte hai!")
            return
...
async def mute_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        chat_member = await update.effective_chat.get_member(update.effective_user.id)
        if chat_member.status not in ['administrator', 'creator']:
            await update.message.reply_text("Sirf admins mute kar sakte hai babe!")
            return
...
async def unmute_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        chat_member = await update.effective_chat.get_member(update.effective_user.id)
        if chat_member.status not in ['administrator', 'creator']:
            await update.message.reply_text("Sirf admins unmute kar sakte hai babe!")
            return
